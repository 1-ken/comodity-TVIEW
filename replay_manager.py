...
class ReplayManager:
    ...
    def start_replay(
        ...
        logger.info(
            f"Started replay: {self.total_snapshots} snapshots, "
            f"start={start_index}, speed={self.speed}x"
        )
    ...
    def pause(self) -> Dict[str, Any]:
        ...
        if self.state == ReplayState.PLAYING:
            ...
            logger.info(f"Replay paused at snapshot {self.current_index}/{self.total_snapshots}")
        return self.get_status()
    ...
    def seek_to_index(self, index: int) -> Dict[str, Any]:
        ...
        if 0 <= index < self.total_snapshots:
            ...
            logger.info(f"Seek to snapshot {index}/{self.total_snapshots}")
        return self.get_status()
    ...
    def seek_to_percentage(self, percentage: float) -> Dict[str, Any]:
        ...
        if self.total_snapshots > 0:
            ...
            logger.info(f"Seek to {percentage}% (snapshot {self.current_index})")
        return self.get_status()
