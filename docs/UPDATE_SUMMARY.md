...
### 1. **Fixed Critical Bug in app/services/alerts.py**
- **Issue**: `AttributeError: 'list' object has no attribute 'items'`
- **Root Cause**: `storage/alerts.json` was an empty list `[]`, but code expected a dict
- **Solution**: Added logic to handle both legacy list format and current dict format
- **Location**: [app/services/alerts.py](app/services/alerts.py#L49-L75)
...
#### Updated Files:
- **metadata/config.json**: New selectors for TradingView structure
- **app/services/observer.py**: Rewritten `snapshot()` method to extract pairs dynamically
- **app/services/extract_pairs.py**: NEW - Utility script to demonstrate pair extraction
...
| File | Status | Changes |
|------|--------|---------|
| `app/services/alerts.py` | ✏️ Modified | Fixed alert loading logic (lines 49-75) |
| `metadata/config.json` | ✏️ Modified | Updated selectors for new HTML structure |
| `app/services/observer.py` | ✏️ Modified | Rewrote snapshot() method for new structure |
...
1. **Update metadata/config.json** with your target URL
2. **Adjust selectors** in metadata/config.json if the HTML structure changes
3. **Run app/services/extract_pairs.py** to test extraction before deploying
