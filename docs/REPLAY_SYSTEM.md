...
1. **app/services/price_history.py** - Stores price snapshots in JSON format
2. **app/services/replay_manager.py** - Controls playback (play, pause, speed, seek)
3. **app/main.py** - API endpoints and WebSocket integration
4. **client.html** - Web UI with replay controls

### Data Storage

Price history is stored in `storage/price_history.json` with the following format:
