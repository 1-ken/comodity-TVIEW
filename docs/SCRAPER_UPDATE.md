...
### 1. **Configuration File** (`metadata/config.json`)
...
### 2. **Observer Module** (`app/services/observer.py`)
...
### 3. **New Extraction Script** (`app/services/extract_pairs.py`)
...
```bash
python3 -m py_compile app/services/observer.py app/services/alerts.py app/main.py
```

## Fixed Issues

✅ Fixed AttributeError in app/services/alerts.py when loading empty storage/alerts.json (handles both list and dict formats)
